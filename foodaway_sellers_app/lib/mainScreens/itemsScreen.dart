...
import 'package:foodaway_sellers_app/UploadScreens/items_upload.dart';
...
import 'package:foodaway_sellers_app/widgets/items_design.dart';
...
class _ItemsScreenState extends State<ItemsScreen> {
  ...
  Future<String> getNameFromSharedPreferences() async {
    ...
    // itemID = sharedPreferences.getString('Uid') ?? 'Address not found';
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(onPressed:() {
            Navigator.push(context, MaterialPageRoute(builder:(context) => ItemsUpload(model: widget.model,),));
          ...
          builder:(context, snapshot) {
            if(snapshot.connectionState == ConnectionState.waiting){
              ...
              return CustomScrollView(
                  slivers: [
                    SliverPersistentHeader(pinned: true ,delegate: HeaderText(headerText: 'My ' + widget.model!.menuTitle.toString() + "'s items ")),
          StreamBuilder<QuerySnapshot>(
                    stream: FirebaseFirestore.instance.collection("sellers")
                    .doc(sellerUid).collection("menus").
                    doc(widget.model!.menUID).collection("items").orderBy("publishedDate",descending: true).snapshots(),
                      ...
                      return !snapshot.hasData ? SliverToBoxAdapter(child: Center(child: CircularProgressIndicator()),)
                       : SliverGrid(
                        delegate:SliverChildBuilderDelegate(
                          ...
                          return ItemsDesignDisplay(model:model,context: context,);
                        },
